...
class ImprovedFinancialExtractor:
    ...
    def extract_financial_analysis(self, event: Dict, trace: Dict, nft_address: str, is_mint: bool = False) -> FinancialAnalysis:
        ...
        # Initialize flags for seller-rescue logic and Fragment detection
        self._has_seller_credits = False
        self._fragment_nft_payment_detected = False
        
        # Store event_id for scope creation and event for buyer wallet detection
        self._current_event_id = event_id
        self._current_event = event
        
        # Use provided mint flag or detect from event
        self._is_mint_event = is_mint or self._detect_mint_from_event(event)
        if self._is_mint_event:
        ...
        if hasattr(self, '_last_path_flows') and self._last_path_flows:
            for path_flow in self._last_path_flows:
        ...
        self.fee_hits = []
        self._seen_fee_keys = set()  # Global deduplication: (marketplace, address, lt) keys
        ...
        if existing_flow_fees == 0 or len(getattr(self, '_seen_fee_keys', set())) == 0:
            # Scoped scan: strict NFT window
            scoped_hits_before = len(self.fee_hits)
            self._collect_fee_hits_in_window(
                trace,
                scope.allowed_node_ids,
                scope.start_lt,
                scope.end_lt,
                use_allowed_nodes=False,  # Allow fee collection outside NFT path
                within_scope=True
            )
            scoped_hits = len(self.fee_hits) - scoped_hits_before
            
            # Relaxed scan: extended window for post-settlement fees
            fee_lt_slack = 200  # ~2 blocks buffer for post-settlement fees
            relaxed_hits_before = len(self.fee_hits)
            self._collect_fee_hits_relaxed(
                trace,
                scope.sale_contract_addr,
                scope.start_lt,
                scope.end_lt + fee_lt_slack
            )
        ...
        nft_credit = next(
            (flow.net_amount_ton for flow in net_flows
             if self._normalize_address(flow.address) == self._normalize_address(nft_address)
        ...
        if not buyer:
            ...
            buyer_fallback = None
            if self._is_mint_event:
                buyer_fallback = self._extract_buyer_from_teleitem_notification(trace)
                if buyer_fallback:
            ...
            if buyer_fallback and self._normalize_address(buyer_fallback) not in exclude:
        ...
        teleitem_bid_amount = self._extract_bid_amount_from_teleitem(trace) if (self._is_mint_event or has_fragment_signal) else None
        if teleitem_bid_amount:
            buyer_payment = teleitem_bid_amount
            self._teleitem_buyer_payment = teleitem_bid_amount  # Store TeleitemBidInfo amount separately
        ...
        if transfer_result.success:
            ...
            if getattr(transfer_result, "from_address", ""):
                ...
                if self._normalize_address(cand) not in guard:
                    if seller != cand:
                        ...
                        if is_fragment_sale:
                            ...
                            seller_gets_auction_fillup = self._check_auction_fillup_recipient(trace, cand)
            ...
            if transfer_result.to_address:
                ...
                if scope and scope.sale_contract_addr:
                    if self._normalize_address(candidate_buyer) == self._normalize_address(scope.sale_contract_addr):
                ...
                if scope and scope.nft_address:
                    if self._normalize_address(candidate_buyer) == self._normalize_address(scope.nft_address):
        ...
        if sale_amount == 0:
            ...
            if actual_seller_credits > 0:
                ...
                if any(h.marketplace_type == 'fragment' for h in fee_hits) and seller:
                    seller_fillup = self._get_auction_fillup_amount(trace, seller)
                    if seller_fillup > 0:
        ...
        if buyer and scope and scope.sale_contract_addr:
            if self._normalize_address(buyer) == self._normalize_address(scope.sale_contract_addr):
                # Fragment exception: Allow buyer == sale_contract if Fragment evidence exists
                if has_fragment_evidence:
        ...
        scope = getattr(self, '_current_analysis_scope', None)
        if seller and buyer and self._normalize_address(seller) == self._normalize_address(buyer):
    ...
    def _extract_nft_specific_flows_from_trace(self, trace: Dict, nft_address: str) -> List[Dict]:
        ...
        self.logger.debug(f"   🧱 CONTEXT: by_dest={len(self._last_by_dest_index)} sale_contract={self._last_sale_contract_address[-10:] if self._last_sale_contract_address else None}")
        
        # Step 4: Add buyer payment flow (use topmost external wallet, not trace root)
        event = getattr(self, '_current_event', {})  # Get full event from context
    ...
    def _validate_nft_path(self, flows: List[Dict], nft_address: str, sale_contract_addr: str, is_mint: bool = False, relaxed: bool = False) -> bool:
        ...
        self._has_seller_credits = has_seller_credits
    ...
    def _find_nft_seed_nodes(self, trace: Dict, nft_address: str) -> List[Dict]:
        ...
        self._traverse_trace_tree(trace, search_node)
        return seeds
    ...
    def _trace_upward_to_buyer(self, trace: Dict, nft_seeds: List[Dict], nft_address: str) -> tuple:
        ...
        if not sale_contract_candidates:
            ...
            if not sale_contract_candidates:
                ...
                for node_addr in all_nodes:
                    if not self._normalize_address(node_addr) == self._normalize_address(nft_address):
            ...
            if matched_candidate:
                try:
                    ...
                    if dest_addr == nft_address:
                        ...
                        self._fragment_nft_payment_detected = True
                        self._last_buyer_payment = int(value_nanoton)  # Preserve buyer payment before validation
        ...
        fragment_payment = self._find_fragment_auction_fillup_payment(trace, sale_contract_candidates, nft_address)
        if fragment_payment:
            return fragment_payment
        
        # B) Use TeleitemBidInfo as price source if available
        teleitem_payment = self._extract_teleitem_bid_amount(trace, nft_address)
        if teleitem_payment:
            ...
            if fragment_beneficiary:
                self.logger.debug(f"   💰 Using TeleitemBidInfo amount {teleitem_payment[1]/1e9:.6f} TON with Fragment beneficiary {fragment_beneficiary[-20:]}")
                return fragment_beneficiary, teleitem_payment[1]
            # Fallback to first sale contract candidate
            elif sale_contract_candidates:
                first_candidate = list(sale_contract_candidates)[0]
                self.logger.debug(f"   💰 Using TeleitemBidInfo amount {teleitem_payment[1]/1e9:.6f} TON with sale candidate {first_candidate[-20:]}")
                return first_candidate, teleitem_payment[1]
        
        return None, 0
    
    def _find_fragment_auction_fillup_payment(self, trace: Dict, sale_contract_candidates: set, nft_address: str) -> tuple:
        """Find auction_fill_up (0x370fec51) operations in trace and extract payment amounts"""
        self.logger.debug(f"   🔍 Scanning for auction_fill_up operations...")
        ...
        def scan_node(node):
            ...
            if decoded_op_name == 'auction_fill_up':
                ...
                if credit > 0:
                    account_addr = tx.get('account', {}).get('address', '')
                    self.logger.debug(f"   🎯 Found auction_fill_up: {credit/1e9:.6f} TON at {account_addr[-20:] if account_addr else 'unknown'}")
                    
                    # Check if this connects to any of our sale contract candidates
                    for candidate in sale_contract_candidates:
                        if self._is_connected_to_sale_contract(node, candidate, trace):
                            self.logger.debug(f"   ✅ auction_fill_up connected to sale contract {candidate[-20:]}")
                            return candidate, credit
            
            return None
        
        # Traverse entire trace looking for auction_fill_up
        result = self._traverse_trace_tree_with_result(trace, scan_node)
        return result if result else None
    
    def _extract_teleitem_bid_amount(self, trace: Dict, nft_address: str) -> tuple:
        """Extract bid amount from TeleitemBidInfo in trace"""
        self.logger.debug(f"   🔍 Looking for TeleitemBidInfo in trace...")
        ...
        def scan_node(node):
            ...
            # Check decoded operations for TeleitemBidInfo
            ...
            if 'TeleitemBidInfo' in decoded_body:
                ...
                if isinstance(bid_info, dict) and 'bid' in bid_info:
                    try:
                        ...
                        if bid_amount > 0:
                            self.logger.debug(f"   💰 Found TeleitemBidInfo bid: {bid_amount/1e9:.6f} TON")
                            return "teleitem_bid", bid_amount
                    except (ValueError, TypeError):
                        pass
    ...
    def _find_fragment_beneficiary(self, trace: Dict, nft_address: str) -> str:
        ...
        def scan_node(node):
            ...
            account_addr = tx.get('account', {}).get('address', '')
            if not self._normalize_address(account_addr) == self._normalize_address(nft_address):
                return None
    ...
    def _traverse_trace_tree_with_result(self, trace: Dict, callback) -> any:
        """Traverse trace tree and return first non-None result from callback"""
        def visit(node):
            if not isinstance(node, dict):
                return None
            
            # Try callback on current node
    ...
    def _extract_sale_only_flows(self, trace: Dict, nft_seeds: List[Dict], nft_address: str) -> List[Dict]:
        ...
        if not sale_contract_candidates:
            ...
            fee_hits = self._collect_fee_hits_relaxed(trace, "", start_lt, end_lt)
        ...
        if not flows:
            ...
            if teleitem_payment:
                ...
                flows.append({
                    ...
                    'evidence': f'Inferred from TeleitemBidInfo: {bid_amount / 1e9:.6f} TON'
                })
    ...
    def _trace_downward_from_sale_contract(self, trace: Dict, sale_contract_addr: str, nft_address: str, 
        ...
        def _extract_path_nodes(trace, allowed_node_ids, start_lt, end_lt):
            ...
            self._traverse_trace_tree(trace, visit_node)
            return path_nodes
    ...
    def _precompute_allowed_nodes(self, trace: Dict, nft_seeds: List[Dict]) -> set:
        ...
        self._traverse_trace_tree(trace, index_by_account)
    ...
    def _allow_top_credit_candidate(self, trace: Dict, start_lt: int, end_lt: int, 
        ...
        self._traverse_trace_tree(trace, scan_node)
        ...
        if best[0] is None:
            ...
            self._traverse_trace_tree(trace, scan_fallback)
    ...
    def _build_by_source_index(self, trace: Dict) -> Dict[str, List[Dict]]:
        ...
        self._traverse_trace_tree(trace, index_node)
    ...
    def _build_by_dest_index(self, trace: Dict) -> Dict[str, List[Dict]]:
        ...
        self._traverse_trace_tree(trace, collect_accounts)
    ...
    def _collect_fee_hits_in_window(self, trace: Dict, allowed_nodes: set, start_lt: int, end_lt: int, use_allowed_nodes: bool = False, within_scope: bool = True):
        """Collect fee hits (amount-independent) within LT window"""
        ...
        if hasattr(self, '_nft_specific_fee_context') and self._nft_specific_fee_context:
            return self._collect_fee_hits_nft_specific(trace, allowed_nodes, start_lt, end_lt, use_allowed_nodes, within_scope)
        
        # Fallback to original logic for backwards compatibility
        return self._collect_fee_hits_original(trace, allowed_nodes, start_lt, end_lt, use_allowed_nodes, within_scope)
    ...
    def _scan_messages_for_fee_destinations(self, trace: Dict, fee_whitelist: Dict[str, str], start_lt: int, end_lt: int, within_scope: bool = True):
        ...
        self._traverse_trace_tree(trace, scan_node_messages)
    ...
    def _collect_fee_hits_relaxed(self, trace: Dict, sale_contract_addr: str, start_lt: int, end_lt: int):
        ...
        self._traverse_trace_tree(trace, visit_relaxed_node)
    ...
    def _build_fallback_scope(self, trace: Dict, event: Dict) -> AnalysisScope:
        ...
        self._traverse_trace_tree(trace, collect_nodes_in_window)
    ...
    def _allow_top_credit_candidate_strict(self, trace: Dict, start_lt: int, end_lt: int, 
        ...
        self._traverse_trace_tree(trace, find_sale_utime)
        ...
        self._traverse_trace_tree(trace, scan_strict)
    ...
    def _is_child_of_sale_contract(self, node: Dict, sale_contract_addr: str, fee_addr: str) -> bool:
        ...
        try:
            ...
            def find_sale_node(trace_node):
                ...
                account_addr = tx.get('account', {}).get('address', '')
                if self._normalize_address(account_addr) == self._normalize_address(sale_contract_addr):
                    return trace_node
    ...
    def _setup_nft_specific_fee_context(self, trace: Dict, sale_contract_addr: str, nft_address: str, start_lt: int, end_lt: int):
        ...
        if hasattr(self, '_has_seller_credits') and self._has_seller_credits:
            ...
            self._traverse_trace_tree(trace, find_seller_payout_lt)
        
        # Scan trace for LT values
        self._traverse_trace_tree(trace, find_sale_lt)
        self._traverse_trace_tree(trace, find_nft_credit_lt)
    ...
    def _traverse_trace_tree(self, node: Dict, callback):
        """Recursively traverse the entire trace tree"""
        callback(node)
        
        if 'children' in node:
            for child in node['children']:
                self._traverse_trace_tree(child, callback)
    ...
    def _determine_seller_buyer(self, net_flows: List[NetFlow], marketplace_fees: List[NetFlow]) -> Tuple[str, str, float]:
        ...
        path_addr_set = {self._normalize_address(n.get("address", "")) for n in (self._current_analysis_scope.path_nodes or []) if n.get("address")}
        ...
        guards = {
            ...
            self._normalize_address(self._current_analysis_scope.sale_contract_addr) if getattr(self, '_current_analysis_scope', None) else '',
            self._normalize_address(nft_address or '')
        }
    ...
    def _extract_buyer_from_teleitem_notification(self, trace: Dict) -> Optional[str]:
        """
        Extract buyer address from Fragment mint_auction TeleitemBidInfo notification.
        
        In Fragment mint_auction:
        - Child node sends nft_ownership_assigned to buyer wallet (ABORTED)
        - Message contains TeleitemBidInfo with bid amount
        - The destination address of this message is the buyer wallet
        """
        def scan_node(node):
            if not isinstance(node, dict):
                return None
            ...
            # Check for nft_ownership_assigned message with TeleitemBidInfo
            in_msg = tx.get('in_msg', {})
            if not in_msg:
                return None
                
            # Check if this is an nft_ownership_assigned message
            decoded_op_name = in_msg.get('decoded_op_name', '')
            if decoded_op_name != 'nft_ownership_assigned':
                return None
                
            # Check for TeleitemBidInfo in decoded body
            ...
            # Check if forward_payload contains TeleitemBidInfo
            value = forward_payload.get('value', {})
            if not value:
                return None
                
            sum_type = value.get('sum_type', '')
            if sum_type != 'TeleitemBidInfo':
                return None
                
            # Extract destination address (buyer wallet)
            destination = in_msg.get('destination', {})
            buyer_address = destination.get('address', '')
            
            if buyer_address:
                # Extract bid amount for logging
                bid_value = value.get('value', {})
                bid_amount = bid_value.get('bid', '0')
                try:
                    bid_ton = int(bid_amount) / 1e9
                    self.logger.debug(f"   💰 TeleitemBidInfo: buyer={buyer_address[-10:]} bid={bid_ton:.6f} TON")
                except (ValueError, TypeError):
                    pass
        ...
        # Traverse trace to find TeleitemBidInfo notification
        result = self._traverse_trace_tree_with_result(trace, scan_node)
        return result if result else None

    def _extract_bid_amount_from_teleitem(self, trace: Dict) -> Optional[float]:
        """
        Extract bid amount from Fragment mint_auction TeleitemBidInfo notification.
        Returns the bid amount in TON.
        """
        def scan_node(node):
            if not isinstance(node, dict):
                return None
            ...
            try:
                bid_nanoton = int(bid_amount)
                bid_ton = bid_nanoton / 1e9
                if bid_ton > 0:
                    self.logger.debug(f"   💰 Extracted TeleitemBidInfo bid: {bid_ton:.6f} TON")
                    return bid_ton
            except (ValueError, TypeError):
                pass
    ...
    def _check_auction_fillup_recipient(self, trace: Dict, address: str) -> bool:
        """Check if address receives auction_fill_up in trace"""
        ...
        def check_node(node):
            ...
            account_addr = self._normalize_address(tx.get('account', {}).get('address', ''))
            if account_addr != normalized_addr:
                return False
                
            # Check if receives auction_fill_up
            in_msg = tx.get('in_msg', {})
            if in_msg.get('decoded_op_name', '') == 'auction_fill_up':
                credit = tx.get('credit_phase', {}).get('credit', 0)
                if credit > 0:
                    return True
            
            return False
        
        result = self._traverse_trace_tree_with_result(trace, check_node)
        return bool(result)

    def _get_auction_fillup_amount(self, trace: Dict, address: str) -> float:
        """Get auction_fill_up amount for specific address in trace"""
        normalized_addr = self._normalize_address(address)
        
        def get_fillup_amount(node):
            if not isinstance(node, dict):
                return None
            tx = node.get('transaction', {})
            if not tx:
        ...
        result = self._traverse_trace_tree_with_result(trace, get_fillup_amount)
